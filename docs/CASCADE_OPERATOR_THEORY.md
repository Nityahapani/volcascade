...
**Significance.** This identity is the foundation for the L² convergence argument in Section 5.4. In particular, for iid R, ‖D(R)‖² = E[D(R)²] = Var(R) = ‖R‖² (for zero-mean R). So D is a non-expansive isometry on zero-mean iid L² at the FIRST application. Subsequent applications decrease the norm (see Section 5.4).
